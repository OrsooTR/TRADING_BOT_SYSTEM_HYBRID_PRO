...
# ── Simulation ────────────────────────────────────────────────────────────────
def sim_and_metrics(entry_bars, directions, ep, sl_d, rr, high, low, close, max_hold):
    n = len(entry_bars)
    if n == 0:
...
def run_strategy(long_s, short_s, regime, target_regime, sess_m, valid,
                 op, atr, sl_mult, rr, high, low, close, max_hold, filter_on):
    """
    ...
    return sim_and_metrics(eb, all_d, ep, sl_d, rr, high, low, close, max_hold)
    ...
    for N, delta, slope_thresh in product(FFT_NS[tf], DELTAS, SLOPE_THRESH):
        ...
        for strat_name, sess in product(STRAT_PARAMS.keys(), SESSIONS):
            ...
            base = run_strategy(ls, ss, regime, tgt_r, sm, valid,
                                ...
                                high, low, close, max_hold, filter_on=False)
            # With regime filter
            filt = run_strategy(ls, ss, regime, tgt_r, sm, valid,
                                op, atr, sp["sl_mult"], sp["rr"],
                                high, low, close, max_hold, filter_on=True)
