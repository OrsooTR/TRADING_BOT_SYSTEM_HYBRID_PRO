"""
ATR Channel Breakout — Vectorized Backtester
...
OUT_DIR = ROOT / "artifacts" / "atr_backtests" / "data"
RPT_DIR = ROOT / "artifacts" / "atr_backtests" / "reports"
...
PARAM_COLS = ["ema_period","atr_channel_mult","sl_atr_mult","rr","session_label"]
...
def sim_and_metrics(entry_bars, directions, ep, sl_d, rr, high, low, close, max_hold=200):
    ...
    tp_p = ep + directions*rr*sl_d
    sl_p = ep - directions*sl_d
    long_i  = directions == 1
    short_i = directions == -1
    tp_hit = np.zeros((n,mj),dtype=bool)
    sl_hit = np.zeros((n,mj),dtype=bool)
    if long_i.any():
        tp_hit[long_i]  = H[long_i] >= tp_p[long_i,None]
        sl_hit[long_i]  = L[long_i] <= sl_p[long_i,None]
    if short_i.any():
        tp_hit[short_i] = L[short_i] <= tp_p[short_i,None]
        sl_hit[short_i] = H[short_i] >= sl_p[short_i,None]
    ...
    win  = (tp_b<sl_b)&(tp_b<mj)
    lose = (sl_b<=tp_b)&(sl_b<mj)
    ...
    return {"trades":tot,"win_rate":round(wins/tot*100,2) if tot else 0.,
            ...
            "pnl_pct":round((eq-1)*100,2),"longs":int(long_i.sum()),
            "shorts":int(short_i.sum()),"avg_bars":round(float(exit_b.mean()),1)}

# ── grid ───────────────────────────────────────────────────────────────────────
def run_atr_grid(df, max_hold=200):
    close=df["close"].values; high=df["high"].values
    low=df["low"].values;     op=df["open"].values
    idx=df.index; atr=calc_atr(df); N=len(close)
    ema_cache={p:calc_ema(close,p) for p in EMA_PERIODS}
    sm_cache={s:smask(idx,s) for s in SESSIONS}
    results=[]
    for ema_p,atr_m,sl_m,rr,sess in product(EMA_PERIODS,ATR_MULTS,SL_MULTS,RRS,SESSIONS):
        ev=ema_cache[ema_p]; sm=sm_cache[sess]
        warmup=max(ema_p,14)+1
        valid=np.zeros(N,dtype=bool); valid[warmup:]=True
        act=sm&valid
        up=ev+atr_m*atr; lo=ev-atr_m*atr
        pc=np.roll(close,1); pu=np.roll(up,1); pl=np.roll(lo,1)
        l_sig=act&(pc<=pu)&(close>up); l_sig[0]=False
        s_sig=act&(pc>=pl)&(close<lo); s_sig[0]=False
        li=np.where(l_sig)[0]; si=np.where(s_sig)[0]
        all_i=np.concatenate([li,si])
        all_d=np.concatenate([np.ones(len(li)),- np.ones(len(si))])
        if len(all_i)==0:
            results.append({**dict(zip(PARAM_COLS,[ema_p,atr_m,sl_m,rr,sess])),
        ...
        results.append({**dict(zip(PARAM_COLS,[ema_p,atr_m,sl_m,rr,sess])),**m})
    return results
...
def write_report(label, df, ts):
    ...
    b=df.iloc[0]
    lines=[f"# ATR Channel Breakout — EURUSD {label}","",
        ...
        "## Best Setup",
        f"- TF={b['tf']}  EMA={int(b['ema_period'])}  ATR_mult={b['atr_channel_mult']:.1f}",
    ...
    for i,r in enumerate(df.head(10).itertuples(index=False),1):
        d=r._asdict()
        lines.append(f"| {i} | {d['tf']} | {int(d['ema_period'])} | {d['atr_channel_mult']:.1f}"
    ...
    (RPT_DIR/f"atr_report_{label.lower().replace(' ','_')}.md"
     ).write_text("\n".join(lines)+"\n",encoding="utf-8")
...
def main():
    ...
    ncombos=len(EMA_PERIODS)*len(ATR_MULTS)*len(SL_MULTS)*len(RRS)*len(SESSIONS)
    all_dfs=[]
    for tf in TIMEFRAMES:
        ...
        tr=run_atr_grid(train, mh); os_=run_atr_grid(oos, mh)
        combined=build_combined(tf,tr,os_)
        all_dfs.append(combined)
        combined.to_csv(OUT_DIR/f"atr_results_{tf.lower()}.csv",index=False)
        b=combined.iloc[0]
    ...
    all_df=pd.concat(all_dfs).sort_values(
        ...
        ascending=[False,True,False]).reset_index(drop=True)
    all_df.to_csv(OUT_DIR/"atr_results_all.csv",index=False)
    all_df.to_json(OUT_DIR/"atr_results_all.json",orient="records",indent=2)
    write_report("ALL_TFS",all_df,ts)
    for tf in TIMEFRAMES:
        sub=all_df[all_df["tf"]==tf].reset_index(drop=True)
        if len(sub): write_report(tf,sub,ts)
    print(f"\nATR done. {len(all_df)} total rows.")
